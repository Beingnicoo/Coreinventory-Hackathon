 ...
 */
$pageTitle = 'Edit Purchase Order - CoreInventory';
...
// Get existing items
$itemsStmt = $conn->prepare("SELECT * FROM purchase_order_items WHERE po_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$supplier_id) {
        ...
        try {
            // Update PO
            $stmt = $conn->prepare("UPDATE purchase_orders SET supplier_id = ?, order_date = ?, expected_date = ?, notes = ? WHERE id = ?");
            $stmt->bind_param("isssi", $supplier_id, $order_date, $expected_date, $notes, $id);
            $stmt->execute();
            ...
            $conn->commit();
            header('Location: view_po.php?id=' . $id . '&success=updated');
            exit;
        } catch (Exception $e) {
            $conn->rollback();
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="bi bi-pencil text-primary"></i> Edit Purchase Order: <?= htmlspecialchars($po['po_number']) ?></h2>
    <a href="<?= htmlspecialchars($base) ?>/purchase_orders/view_po.php?id=<?= $id ?>" class="btn btn-outline-secondary">
...
<form method="POST" id="poForm">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="supplier_id" class="form-select" required>
                                ...
                                <option value="<?= $s['id'] ?>" <?= ($po['supplier_id'] == $s['id'] || (isset($_POST['supplier_id']) && $_POST['supplier_id'] == $s['id'])) ? 'selected' : '' ?>>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="date" name="order_date" class="form-control" required value="<?= htmlspecialchars($_POST['order_date'] ?? $po['order_date']) ?>">
                        </div>
                        <div class="col-md-3 mb-3">
                            <label class="form-label">Expected Date</label>
                            <input type="date" name="expected_date" class="form-control" value="<?= htmlspecialchars($_POST['expected_date'] ?? $po['expected_date']) ?>">
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Notes</label>
                        <textarea name="notes" class="form-control" rows="2"><?= htmlspecialchars($_POST['notes'] ?? $po['notes']) ?></textarea>
                    </div>
                </div>
            </div>
                        ...
                        <table class="table" id="itemsTable">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <th id="grandTotal">₹<?= number_format($po['total_amount'], 2) ?></th>
                                    <th></th>
                                </tr>
                            </tfoot>
                        </table>
    ...
    <div class="d-flex gap-2">
        <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg"></i> Update Purchase Order</button>
        <a href="<?= htmlspecialchars($base) ?>/purchase_orders/view_po.php?id=<?= $id ?>" class="btn btn-outline-secondary">Cancel</a>
    </div>
</form>
