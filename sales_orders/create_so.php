 ...
 * Create Sales Order
 */
$pageTitle = 'Create Sales Order - CoreInventory';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$customer_id) {
        ...
        if (!$stockError) {
            // Generate SO number
            $soCount = $conn->query("SELECT COUNT(*) as c FROM sales_orders")->fetch_assoc()['c'] ?? 0;
            $so_number = 'SO-' . date('Y') . '-' . str_pad($soCount + 1, 5, '0', STR_PAD_LEFT);

            $conn->begin_transaction();
            try {
                // Create SO
                $stmt = $conn->prepare("INSERT INTO sales_orders (so_number, customer_id, order_date, delivery_date, notes, created_by, status) VALUES (?, ?, ?, ?, ?, ?, 'draft')");
                $stmt->bind_param("sissis", $so_number, $customer_id, $order_date, $delivery_date, $notes, $userId);
                $stmt->execute();
                ...
                foreach ($items as $item) {
                    ...
                    if ($product_id && $quantity > 0) {
                        ...
                        $itemStmt->bind_param("iiid", $so_id, $product_id, $quantity, $unit_price);
                        $itemStmt->execute();
                ...
                $updateStmt->bind_param("di", $total, $so_id);
                $updateStmt->execute();

                $conn->commit();
                header('Location: view_so.php?id=' . $so_id . '&success=created');
                exit;
            } catch (Exception $e) {
                $conn->rollback();
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="bi bi-plus-circle text-primary"></i> Create Sales Order</h2>
...
<form method="POST" id="soForm">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="date" name="delivery_date" class="form-control" value="<?= htmlspecialchars($_POST['delivery_date'] ?? '') ?>">
                        </div>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="table-responsive">
                        <table class="table" id="itemsTable">
                            ...
                            <tbody id="itemsBody">
                                <tr class="item-row">
                                    <td>
                                        <select name="items[0][product_id]" class="form-select product-select" required onchange="updateProductInfo(this)">
    ...
    <div class="d-flex gap-2">
        <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg"></i> Create Sales Order</button>
        <a href="<?= htmlspecialchars($base) ?>/sales_orders/so_list.php" class="btn btn-outline-secondary">Cancel</a>
    </div>
</form>
